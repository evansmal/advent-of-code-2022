...
(* Read the input *)
let get_input = Common.read_file "data/01.txt"

(* Split the input into blocks *)
let split_blocks input =
  let seg = Common.split_by "\n\n" input in
  let blocks = List.map ~f:(Common.split_by "\n") seg in
...
let part_one =
  let sums = List.map ~f:sum (split_blocks get_input) in
  printf "Part 1: %d\n" (max sums)
